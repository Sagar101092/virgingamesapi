...
public class CurrencyIsEUR extends TestBase {
    public static ValidatableResponse validatableResponse;
    public static Response response;
    @Steps
    VirginGamesApiSteps virginGamesApiSteps;

    @Title("Get All data for Quarry Parameter Currency =  EUR")
    @Test
    public void test01() {

        response = virginGamesApiSteps.getALlDataForEUR();
        validatableResponse = response.then().log().all().statusCode(200);

        Assert.assertEquals("Bingo", validatableResponse.extract().path("data.jackpotId"));
        Assert.assertEquals("EUR", validatableResponse.extract().path("data.pots[0].currency"));
    }
